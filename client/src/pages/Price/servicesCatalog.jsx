
export const artists = [
    {
        img:'./img/Services/Artists/ShowMan.jpeg', 
        name:'Ведущие',
        description:
        `Наши ведущие проведут любое мероприятие от начала и 
        до конца на высшем уровне! Равнодушным не уйдёт никто!`,
        price:'От 7000 ₽ / час',
        photo: [
            {
                card: "./img/services_card_photo/Artists/ShowMan/SM1.jpeg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Artists/ShowMan/SM2.jpeg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Artists/ShowMan/SM3.jpeg",
                id: 3,
            }, 
            // {
            //     card: "./img/Services/Artists/ShowMan.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Artists/ShowMan.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Artists/Artist_Services.jpeg', 
        name:'Артисты',
        description:
        `Мы предлагаем Вам услуги наших артистов (аниматоров) 
        на любое мероприятие - День рождения, корпоратив, вечеринка 
        и т.д. Они с лёгкостью найдут контакт с любым, даже 
        самым скованным ребёнком или взрослым.`,
        price:'2500–3500 ₽ / час',
        photo: [
            {
                card: "./img/services_card_photo/Artists/Artists/Artist1.jpeg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Artists/Artists/Artist2.jpeg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Artists/Artists/Artist3.jpeg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/Artists/Artists/Artist4.jpeg",
                id: 4,
            }, 
            // {
            //     card: "./img/Services/Artists/Artist_Services.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Artists/Illusions.jpeg', 
        name:'Иллюзионисты',
        description:
        `Наши иллюзионисты предлагают захватывающие программы для всех возрастов.
        
        Продолжительность шоу от 30 минут. Микромагия - от 60 минут. К Вашим услугам также мастер-классы по обучению простым и эффектным трюкам.`,
        price:'от 15 000 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Artists/Illusionists/Illus2.jpeg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Artists/Illusionists/Illus1.jpeg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Artists/Illusionists/Illus3.jpeg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/Artists/Illusionists/Illus4.jpeg",
                id: 4,
            }, 
            // {
            //     card: "./img/services_card_photo/Artists/ShowMan/SM1.jpeg",
            //     id: 5,
            // },
        ]
    },
]
export const programms = [ 
    {
        img:'./img/Services/Programms/Show.jpeg', 
        name:'Шоу',
        description:``,
        price:'',
        photo: [
            {
                card: "./img/Services/Programms/Show.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/Programms/Show.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/Programms/Show.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/Programms/Show.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/Programms/Show.jpeg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/Programms/Quest.jpeg', 
        name:'Квесты',
        description:``,
        price:'',
        photo: [
            {
                card: "./img/Services/Programms/Quest.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/Programms/Quest.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/Programms/Quest.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/Programms/Quest.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/Programms/Quest.jpeg",
                id: 5,
            },
        ]
    },
    // {
    //     img:'./img/Services/Programms/Quiz.jpeg', 
    //     name:'Квизы',
    //     description:``,
    //     price:'',
    //     photo: [
    //         {
    //             card: "./img/Services/Programms/Quiz.jpeg",
    //             id: 1,
    //         }, 
    //         {
    //             card: "./img/Services/Programms/Quiz.jpeg",
    //             id: 2,
    //         }, 
    //         {
    //             card: "./img/Services/Programms/Quiz.jpeg",
    //             id: 3,
    //         }, 
    //         {
    //             card: "./img/Services/Programms/Quiz.jpeg",
    //             id: 4,
    //         }, 
    //         {
    //             card: "./img/Services/Programms/Quiz.jpeg",
    //             id: 5,
    //         },
    //     ]
    // },
    {
        img:'./img/Services/Programms/BirthdDay.jpeg', 
        name:'Дни рождения',
        description:``,
        price:'От 30 000 руб',
        photo: [
            {
                card: "./img/Services/Programms/BirthdDay.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/Programms/BirthdDay.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/Programms/BirthdDay.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/Programms/BirthdDay.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/Programms/BirthdDay.jpeg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/Programms/KidOut.jpeg', 
        name:'Детские выпускные',
        description:``,
        price:'',
        photo: [
            {
                card: "./img/Services/Programms/KidOut.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/Programms/KidOut.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/Programms/KidOut.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/Programms/KidOut.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/Programms/KidOut.jpeg",
                id: 5,
            },
        ]
    },
]
export const birthDay = [
    {
        img:'./img/Services/Programms/BirthdDay.jpeg', 
        name:'Базовый',
        description:
        `Базовый – создание сценария, 1-2 аниматора (ведущий), 
        любое шоу на выбор или квест. Beauty-бар или аквагрим, 
        шоколадный фонтан, оформление площадки для 
        праздника, фотоуслуги.`,
        price:'от 30 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/BirthdDay.jpeg', 
        name:'Стандарт',
        description:
        `Стандарт – создание сценария, 1-2 аниматора 
        (ведущий), любое шоу на выбор или квест. Beauty-бар и аквагрим, шоколадный 
        фонтан, оформление площадки для праздника,  
        дискотека, организация фуршета, фото или видео услуги.`,
        price:'от 50 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/BirthdDay.jpeg', 
        name:'VIP',
        description:
        `VIP – создание сценария, 2-3 аниматора (ведущий), 
        любое шоу на выбор или квест. 
        Beauty-бар и аквагрим, проведение мастер-классов, шоколадный 
        фонтан, оформление площадки для праздника, 
        фотозона, дискотека с DJ, организация фуршета, фото 
        и видео услуги.`,
        price:'от 80 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/BirthDay/BD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Programms/BirthdDay.jpeg",
            //     id: 5,
            // },
        ]
    },
]
export const outDay = [
    {
        img:'./img/Services/Programms/KidOut.jpeg', 
        name:'Базовый',
        description:
        `Создание сценария, 2-3 аниматора (ведущий), 
        любое шоу на выбор или квест. Beauty-бар и аквагрим, 
        шшоколадный фонтан, 
        оформление площадки для праздника, фото и видео услуги. `,
        price:'от 45 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/KidOut.jpeg', 
        name:'Стандарт',
        description:
        `Создание сценария, 2-3 аниматора 
        (ведущий), любое шоу на выбор или квест. Beauty-бар и аквагрим, шоколадный фонтан, оформление площадки 
        для праздника, дискотека, организация фуршета, 
        фото и видео услуги.`,
        price:'от 60 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/KidOut.jpeg', 
        name:'VIP',
        description:
        `Создание сценария, 2-4 аниматора (ведущий), 
        любое шоу на выбор или квест. 
        Beauty-бар и аквагрим, проведение мастер-классов, 
        представление с шоколадным фонтаном и фруктами, 
        оформление площадки для праздника, дискотека с 
        DJ, организация фуршета, фото и видео услуги.`,
        price:'от 80 000 руб',
        photo: [
            {
                card: "./img/services_card_photo/Programs/OutDay/OD1.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/Services/Programms/KidOut.jpeg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/Services/Programms/KidOut.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/Services/Programms/KidOut.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Programms/KidOut.jpeg",
            //     id: 5,
            // },
        ]
    },
]
export const quests = [
    {
        img:'./img/Services/Programms/Quest.jpeg', 
        name:'Тайник Флинта',
        description:
        `Развлекательно-образовательный квест.
        Вас ждут приключения и препятствия, а так же сувениры 
        от пиратов, тематическая фотозона 
        и дополнительно – трапеза в таверне. 
        Продолжительность 
        – от 1,5 часов.`,
        price:'от 30000 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Programs/Quests/Flint/Pir1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Flint/Pir2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Flint/Pir3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/Services/Programms/Quest.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Programms/Quest.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/Quest.jpeg', 
        name:'В поисках сокровища Форта',
        description:
        `Приключенческий квест на логику, смекалку, силу и 
        выносливость! Время игры около 1,5 часов. 
        Дополнительно – футболки с логотипом игры, подарок 
        от агентства.`,
        price:'от 20000 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Programs/Quests/Fort/TF1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Fort/TF2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Fort/TF3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Fort/TF4.jpg",
                id: 4,
            }, 
            {
                card: "./img/services_card_photo/Programs/Quests/Fort/TF5.jpg",
                id: 5,
            },
        ]
    },
]
export const shows = [
    {
        img:'./img/Services/Programms/Show.jpeg', 
        name:'Научное шоу',
        description:`Наука и чудеса. Учавствуйте в экспериментах и 
        почувствуйте себя частью шоу, приготовьте мороженное из азота!`,
        price:`от 9000 ₽`,
        photo: [
            {
                card: "./img/services_card_photo/Programs/Shows/Alchym/AS1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Alchym/AS2.jpeg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Alchym/AS3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/Programs/Shows/Alchym/OD1.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/Shows/Alchym/OD1.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/Show.jpeg', 
        name:'Шоу мыльных пузырей',
        description:
        `Интерактивное шоу для непосед с эффектными трюками - дым, огонь.
         Музыка танцы и возможность загадать желание внутри большого мыльного пузыря. 
         Продолжительность 30 минут`,
        price:'от 12000 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Programs/Shows/Bubble/BS1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Bubble/BS2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Bubble/BS3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/Programs/Shows/Bubble/BS.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Programs/Shows/Bubble/BS.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/Programms/Show.jpeg', 
        name:'Бумажное шоу',
        description:
        `В программу включено музыкальное 
        оформление и три залпа конфетти.
        Цена указана за 40 минут и изменяется в зависимости 
        от количества килограмм бумаги (минимально – 5 кг) 
        и вида бумажных лент (Белоснежное, Серебряное 
        или Золотое, Цветное).`,
        price:'От 6000 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Programs/Shows/Paper/PS1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Paper/PS2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Programs/Shows/Paper/PS3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/Services/Programms/Show.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/Services/Programms/Show.jpeg",
            //     id: 5,
            // },
        ]
    },
]
export const newYear = [
    {
        img:'./img/Services/NewYear/InteractiveSpectacle.jpeg', 
        name:'Интерактивный спектакль',
        description:
        `Для детских учреждений – «В поисках Нового года». Дед Мороз и Снегурочка, 
        плюс два-три  аниматора-персонажа. Продолжительность 40 минут. `,
        price:'от 30 000 ₽',
        photo: [
            {
                card: "./img/Services/NewYear/InteractiveSpectacle.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/NewYear/InteractiveSpectacle.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/NewYear/InteractiveSpectacle.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/NewYear/InteractiveSpectacle.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/NewYear/InteractiveSpectacle.jpeg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/NewYear/DedMoroz.jpeg', 
        name:'Дед Мороз и Снегурочка',
        description:``,
        price:'',
        photo: [
            {
                card: "./img/Services/NewYear/DedMoroz.jpeg",
                id: 1,
            }, 
            {
                card: "./img/Services/NewYear/DedMoroz.jpeg",
                id: 2,
            }, 
            {
                card: "./img/Services/NewYear/DedMoroz.jpeg",
                id: 3,
            }, 
            {
                card: "./img/Services/NewYear/DedMoroz.jpeg",
                id: 4,
            }, 
            {
                card: "./img/Services/NewYear/DedMoroz.jpeg",
                id: 5,
            },
        ]
    },
]
export const forSell = [
    {
        img:'./img/Services/ForSell/PhotoAndVideo.jpeg', 
        name:'Фото и Видео услуги',
        inServises: [
            'Все виды фотосъемки' ,
            'от 5000 ₽ час',
            'Все виды видеосъемки',
            'от 10 000 ₽ час'
        ], 
        description: '',
        price:'',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/photo_and_video/pav.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photo_and_video/PhotoAndVideo.jpeg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photo_and_video/PhotoAndVideo.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photo_and_video/PhotoAndVideo.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photo_and_video/PhotoAndVideo.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/ForSell/PhotoZone.jpeg', 
        name:'Фотозона',
        description:
            `С шарами, пайетками и многое другое – на любой праздник!
            Изготовление – от 7500 ₽ в зависимости от сроков, объема, сложности исполнения.`,
        price:'Аренда – от 7500 ₽ в сутки',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/photozone/PZ1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/photozone/PZ2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/photozone/PZ3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photozone/PZ4.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/photozone/PZ5.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/ForSell/AquaGrimmAndBeautyBar.jpeg', 
        name:'Аквагрим и Бьюти-бар',
        inServises: [
            '',
            'Аквагрим - от 6000 руб',
            'Beauty-бар - от 8000 руб',
            'Дополнительный стилист - 4500 руб',
            'Дополнительный час - 3000 руб ',
            'Продолжительность - 2 часа',
        ],
        description: '',
        price:'от 6000 ₽ за услугу',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/aqua_and_beauty/AaB1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/aqua_and_beauty/AaB2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/aqua_and_beauty/AaB3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/aqua_and_beauty/AaB4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/aqua_and_beauty/AaB5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/ForSell/Pinyata.jpeg', 
        name:'Пиньята',
        description:
            `На любой вкус и праздник! В подарок – формат игры 
            с Пиньятой. Цена зависит от размера, 
            сложности и наполнения. В услугу входит - создание пиньяты, бита, 
            конфеты и конфетти.`, 
        price:'От 3500 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/pinyata/pin1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/pinyata/pin2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/pinyata/pin3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/pinyata/pin4.jpg",
                id: 4,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/pinyata/pin5.jpg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/ForSell/CakePops2.jpeg', 
        name:'Кейк-попсы',
        description:
            `Кейк-попсы - это «торт на палочке».`,
        price:'От 100 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/KP/KP1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/KP/KP2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/KP/KP3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/KP/KP4.jpg",
                id: 4,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/KP/KP5.jpg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/ForSell/BentaCake.jpeg', 
        name:'Бенто-торты',
        description:
            `Нежнейший крем-чиз, любая начинка и 3 размера. Любые ингриденты, всегда
             в бенто-боксе с ложечкой. Дополнительно: подарочная упаковка, 
            свечки, подарочная открытка ручной работы.`,
        price:'От 700 ₽',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/BT/BT1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/BT/BT2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/Other_Services/BT/BT3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/BT/BT4.jpg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/BT/BT5.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/ForSell/DJ.jpeg', 
        name:'DJ',
        description:
            `Музыка на любой вкус, полное техническое сопровождение 
            (компьютер, микшер, колонки, микрофоны)`,
        price:'От 6000₽ в час ',
        photo: [
            {
                card: "./img/services_card_photo/Other_Services/DJ.jpg",
                id: 1,
            }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/DJ.jpeg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/DJ.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/DJ.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/Other_Services/DJ.jpeg",
            //     id: 5,
            // },
        ]
    },
    // {
    //     img:'./img/Services/ForSell/Pryaniki.jpeg', 
    //     name:'Архангельские пряники',
    //     description:``,
    //     price:'',
    //     photo: [
    //         {
    //             card: "./img/services_card_photo/Other_Services/Pryaniki",
    //             id: 1,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/Other_Services/Pryaniki",
    //             id: 2,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/Other_Services/Pryaniki",
    //             id: 3,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/Other_Services/Pryaniki",
    //             id: 4,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/Other_Services/Pryaniki",
    //             id: 5,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/ForSell/Pryaniki",
    //             id: 6,
    //         }
    //     ]
    // },
]
export const imgMK = [
    {
        img:'./img/Services/MK/CatchSleep.jpeg', 
        name:'Ловцы снов',
        description:
            `Вместе с нами Вы не только сможете сделать 
            "Ловец снов", но и узнаете особенности его создания и историю, 
            легенды. 
            Время проведения мастер-класса от 1 часа до 2,5 часов (в зависимости 
            от сложности поделки). Расходные материалы включены в стоимость.`,
        price:'От 1300р  с человека',
        photo: [
            {             
                card: "./img/services_card_photo/MK/LS/ls1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/LS/ls2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/MK/LS/ls3.jpg",
                id: 3,
            },
            {             
                card: "./img/services_card_photo/MK/LS/ls4.jpg",
                id: 4,
            }, 
            {
                card: "./img/services_card_photo/MK/LS/ls5.jpg",
                id: 5,
            },
        ]
    },
    {
        img:'./img/Services/MK/ClothDecoration.jpeg', 
        name:'Декорирование одежды',
        description:
            `Футболки, сумки, бейсболки и многое другое с помощью несмываемых 
            красок, специальных фломастеров и элементов декора превращаем в 
            уникальный элемент вашего гардероба. Продолжительность - 1 час. 
            Расходные материалы включены в стоимость.`,
        price:'От 1200р с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/Cloth/Cl1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/Cloth/Cl2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/MK/Cloth/Cl3.jpg",
                id: 3,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/Cloth/Cl.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Cloth/Cl.jpeg",
            //     id: 5,
            // },
        ]
    },
    // {
    //     img:'./img/Services/MK/Florarium.jpeg', 
    //     name:'Флорариум',
    //     description:
    //         `Флорариум - это целый мир внутри стеклянной емкости. Туда можно 
    //         поселить гномов, причудливых существ, а деревьями и растительностью 
    //         будут служить необычные суккуленты и мхи. Добавим разноцветный песок 
    //         и камни для фактуры. У нас выйдет замечательный садик с живыми, но 
    //         неприхотливыми цветами. Это познакомит нас с жизнью флоры и привьёт 
    //         ответственность за хрупким миром природы. Материал входит в услугу 
    //         Время проведения мастер-класса от 30 минут до 1 часа. Расходные 
    //         материалы включены в стоимость.`,
    //     price:'От 2500р с человека',
    //     photo: [
    //         {
    //             card: "./img/services_card_photo/MK/Flw/Flw1.jpg",
    //             id: 1,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Flw/Flw2.jpeg",
    //             id: 2,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Flw/Flw3.jpeg",
    //             id: 3,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Flw/Flw4.jpg",
    //             id: 4,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Flw/Flw5.jpg",
    //             id: 5,
    //         },
    //     ]
    // },
    {
        img:'./img/Services/MK/Pryaniki2.jpeg', 
        name:'Архангельские пряники',
        description:
            `Пряники нашей ручной работы расписываем специально подготовленной 
            глазурью с помощью кистей и кондитерских мешочков. Продолжительность  
            от 1,5 до 2 часов. Расходные материалы включены в стоимость.`,
        price:'От 1000р  с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/Pryan/Pryan1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/Pryan/Pryan2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/MK/Pryan/Pryan3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/MK/Pryan/Pryan4.jpg",
                id: 4,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/Pryan/Pryan.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/MK/CakePops.jpeg', 
        name:'Кейк-попсы',
        description:
            `Вместе с нами вы можете приготовить своими ручками кейк-попсы 
            и красиво их упаковать.
            Продолжительность от 1,5 до 2 часов. Расходные материалы 
            включены в стоимость.`,
        price:'От 1500р с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/KP/KP1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/KP/KP2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/MK/KP/KP3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/MK/KP/KP4.jpg",
                id: 4,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/KP/KP.jpg",
            //     id: 5,
            // },
        ]
    },
    // {
    //     img:'./img/Services/MK/Slime.jpeg', 
    //     name:'Слаймы',
    //     description:
    //         `Создадим с детишками слаймы любых цветов и размеров. Также покажем, 
    //         как с ними можно играть. Время проведения мастер-класса 1 час. 
    //         Расходные материалы включены в стоимость.`,
    //     price:'1100р с человека',
    //     photo: [
    //         {
    //             card: "./img/services_card_photo/MK/Slime/Slime1.jpg",
    //             id: 1,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Slime/Slime2.jpeg",
    //             id: 2,
    //         }, 
    //         {
    //             card: "./img/services_card_photo/MK/Slime/Slime3.jpeg",
    //             id: 3,
    //         }, 
    //         // {
    //         //     card: "./img/services_card_photo/MK/Slime/Slime4.jpeg",
    //         //     id: 4,
    //         // }, 
    //         // {
    //         //     card: "./img/services_card_photo/MK/Slime/Slime5.jpeg",
    //         //     id: 5,
    //         // },
    //     ]
    // },
    {
        img:'./img/Services/MK/SinelnayaRipe.jpeg', 
        name:'Синельная проволока',
        description:
            `Из пушистой проволоки Вы можете сотворить любой декор или игружку. Продолжительность  
            от 30 минут до 1,5 часа. 
            Расходные материалы включены в стоимость.`,
        price:'От 1000р с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/SP/SP1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/SP/SP2.jpg",
                id: 2,
            }, 
            {
                card: "./img/services_card_photo/MK/SP/SP3.jpg",
                id: 3,
            }, 
            {
                card: "./img/services_card_photo/MK/SP/SP4.jpg",
                id: 4,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/SP/SP5.jpg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/MK/Capitoshki.jpeg', 
        name:'Капитошки',
        description:
            `Антистрессовые игрушки "Капитошки" отлично снимают напряжение и улучшают настроение.
            Продолжительность - 1 час. Расходные материалы включены 
            в стоимость.`,
        price:'От 700р с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/Kap/Kap1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/Kap/Kap2.jpg",
                id: 2,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/Kap/Kap3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Kap/Kap4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Kap/Kap5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./img/Services/MK/Tvisting.jpeg', 
        name:'Твистинг',
        description:
            `Зрелищный и эффектный мастер-класс путем скручивания шариков. 
            Продолжительность - 1 час. Расходные материалы включены в стоимость.`,
        price:'500р с человека',
        photo: [
            {
                card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
                id: 1,
            }, 
            {
                card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
                id: 2,
            }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },
]

export const toDo = [
    {
        img:'./logo512.png', 
        name:'Гендерпати',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./logo512.png', 
        name:'Тематические квизы',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./logo512.png', 
        name:'Тимбилдинг',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },{
        img:'./logo512.png', 
        name:'Школа Деда Мороза',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./logo512.png', 
        name:'Интерактивный спектакль "Велесова ночь: врата времени"',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },
    {
        img:'./logo512.png', 
        name:'Интерактивный спектакль "Три шляпы"',
        description:
            ``,
        price:'',
        photo: [
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws1.jpg",
            //     id: 1,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws2.jpg",
            //     id: 2,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws3.jpeg",
            //     id: 3,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws4.jpeg",
            //     id: 4,
            // }, 
            // {
            //     card: "./img/services_card_photo/MK/Tws/Tws5.jpeg",
            //     id: 5,
            // },
        ]
    },

]
